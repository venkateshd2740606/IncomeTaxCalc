package com.incometaxcalc.data.repository

import com.incometaxcalc.R
...
import com.incometaxcalc.data.mapper.DataMappers
...
import com.incometaxcalc.engine.PuzzleProfileEngine
import com.incometaxcalc.engine.IncomeTaxCalcEngine
import com.incometaxcalc.engine.IncomeTaxCalcGenerator
import com.incometaxcalc.engine.TutorialLevels
import com.incometaxcalc.util.DateUtils
...
class GameRepositoryImpl @Inject constructor(
    ...
    override suspend fun createNewGame(difficulty: Difficulty, levelNumber: Int): IncomeTaxCalcGame {
        ...
        val seed = IncomeTaxCalcGenerator.seedFromLevelNumber(levelNumber, difficulty)
        val level = IncomeTaxCalcGenerator.generate(seed, levelNumber, difficulty, generationProfile)
        return IncomeTaxCalcEngine.createInitialGame(level)
    }
    ...
    override suspend fun createGameFromSeed(
        ...
        val level = IncomeTaxCalcGenerator.generate(seed, levelNumber, difficulty)
        return IncomeTaxCalcEngine.createInitialGame(level)
    }

    override suspend fun createTutorialGame(tutorialIndex: Int): IncomeTaxCalcGame? {
        val level = TutorialLevels.getTutorialLevel(tutorialIndex) ?: return null
        return IncomeTaxCalcEngine.createInitialGame(level)
    }

    override suspend fun createEndlessGame(wave: Int): IncomeTaxCalcGame {
        val seed = wave.toLong() * 7919L + System.currentTimeMillis() % 1000
        val level = IncomeTaxCalcGenerator.generate(seed, wave, Difficulty.ENDLESS)
        return IncomeTaxCalcEngine.createInitialGame(level)
    }

    override suspend fun saveGame(game: IncomeTaxCalcGame): Long {
    ...
    override suspend fun getGame(gameId: Long): IncomeTaxCalcGame? =
        gameDao.getById(gameId)?.let { DataMappers.fromEntity(it) }

    override suspend fun getInProgressGame(): IncomeTaxCalcGame? =
        gameDao.getInProgress()?.let { DataMappers.fromEntity(it) }

    override fun observeInProgressGame(): Flow<IncomeTaxCalcGame?> =
        gameDao.observeInProgress().map { it?.let { e -> DataMappers.fromEntity(e) } }

    override suspend fun completeGame(game: IncomeTaxCalcGame): IncomeTaxCalcGame {
    ...
    override suspend fun getLevel(seed: Long, levelNumber: Int, difficulty: Difficulty): IncomeTaxCalcLevel =
        IncomeTaxCalcGenerator.generate(seed, levelNumber, difficulty)
}
...
class ChallengeRepositoryImpl @Inject constructor(
    ...
    override suspend fun getChallengeGame(record: ChallengeRecord): IncomeTaxCalcGame {
        val levelNumber = record.key.hashCode().and(0x7FFFFFFF) % 5000 + 1
        val level = IncomeTaxCalcGenerator.generateForChallenge(
...
class ProgressionRepositoryImpl @Inject constructor(
    ...
    override suspend fun updateStatsAfterGame(game: IncomeTaxCalcGame) {
    ...
    private suspend fun updatePuzzleProfile(game: IncomeTaxCalcGame) {
    ...
    private fun updateFastest(current: Long, game: IncomeTaxCalcGame, difficulty: Difficulty): Long {
...
class PreferencesRepositoryImpl @Inject constructor(
    private val dataStore: com.incometaxcalc.data.local.PreferencesDataStore
) : com.incometaxcalc.domain.repository.PreferencesRepository {

    override fun getUserPreferences() = dataStore.preferencesFlow

    override suspend fun updatePreferences(
        transform: (com.incometaxcalc.domain.model.UserPreferences) -> com.incometaxcalc.domain.model.UserPreferences
